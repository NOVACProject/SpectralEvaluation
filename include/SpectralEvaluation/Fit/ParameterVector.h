...
public:
    ...
    */
    bool SetSize(int iSize)
    {
        ...
        if (iSize > 0)
        {
            ...
            mIndex.resize(size); // Was: mIndex = new int[iSize];
            mBackIndex.resize(size); // Was: mBackIndex = new int[iSize];
            ...
            {
                mIndex[static_cast<size_t>(i)] = i;
                mBackIndex[static_cast<size_t>(i)] = i;
            }
        }
    ...
    */
    bool FixParameter(int iParamID)
    {
        ...
        const size_t parameterIdx = static_cast<size_t>(iParamID);

        // check wheter the parameter is already fixed (index == -1)
        int iIndex = mIndex[parameterIdx];
        if (iIndex != -1)
        {
            // no, its not, so prepare index
            mIndex[parameterIdx] = -1;

            // shift the upper indices downwards
            for (int i = iParamInternalSize - 1; i >= iParamID + 1; i--)
            {
                // correct neccessary indices
                if (mIndex[static_cast<size_t>(i)] != -1)
                {
                    mIndex[static_cast<size_t>(i)] -= 1;
                    mBackIndex[static_cast<size_t>(iIndex)] = i;
                }
            }
    ...
    */
    bool ReleaseParameter(int iParamID)
    {
        ...
        const size_t parameterIdx = static_cast<size_t>(iParamID);
        if (mIndex[parameterIdx] != -1)
            return true;

        // search for next available index
        int iIndex = mParams.GetSize();
        for (size_t i = static_cast<size_t>(iParamInternalSize) - 1; i >= parameterIdx + 1; i--)
        {
            if (mIndex[i] != -1)
            {
                iIndex = mIndex[i]++;
        ...
        // set the new index
        mIndex[parameterIdx] = iIndex;
        mBackIndex[static_cast<size_t>(iIndex)] = iParamID;
    ...
    */
    ...
    {
        assert(iParamID >= 0 && iParamID < mParamsInternal.GetSize());

        return mIndex[static_cast<size_t>(iParamID)] == -1;
    }
    ...
    */
    ...
    {
        assert(iParamID >= 0 && iParamID < mParams.GetSize());

        return mBackIndex[static_cast<size_t>(iParamID)];
    }
    ...
    */
    int GetAll2FixedIndex(int iParamID)
    {
        ...
        return mIndex[static_cast<size_t>(iParamID)];
    }
    ...
    */
    bool SetParameters(const CVector& vParams)
    {
        ...
        if (mParamsInternal.GetSize() != iParamSize)
        {
            ...
            for (int i = 0; i < iParamSize; i++)
            {
                mParamsInternal.SetAt(mBackIndex[static_cast<size_t>(i)], mParams.GetAt(i));
            }
        }
        else
        {
    ...
    */
    CVector& GetParameter()
    {
        ...
        if (mParamsInternal.GetSize() != iParamSize)
        {
            for (int i = 0; i < iParamSize; i++)
            {
                mParams.SetAt(i, mParamsInternal.GetAt(mBackIndex[static_cast<size_t>(i)]));
            }
        }
        else
        {
    ...
    */
    bool SetError(const CVector& vError)
    {
        ...
        {
            // copy all unfixed parameters in the internal list
            for (int i = 0; i < iErrorSize; i++)
            {
                mErrorInternal.SetAt(mBackIndex[static_cast<size_t>(i)], mError.GetAt(i));
            }
        }
        else
        {
    ...
    */
    CVector& GetError()
    {
        ...
        if (mErrorInternal.GetSize() != iErrorSize)
        {
            for (int i = 0; i < iErrorSize; i++)
            {
                mError.SetAt(i, mErrorInternal.GetAt(mBackIndex[static_cast<size_t>(i)]));
            }
        }
        else
        {
    ...
    */
    bool SetCorrelMatrix(const CMatrix& mCorrelMatrix)
    {
        ...
        {
            // copy all unfixed parameters in the internal list
            for (int i = 0; i < mCorrel.GetNoRows(); i++)
            {
                for (int j = 0; j < mCorrel.GetNoColumns(); j++)
                {
                    mCorrelInternal.SetAt(mBackIndex[static_cast<size_t>(i)], mBackIndex[static_cast<size_t>(j)], mCorrel.GetAt(i, j));
                    mCorrelInternal.SetAt(mBackIndex[static_cast<size_t>(j)], mBackIndex[static_cast<size_t>(i)], mCorrel.GetAt(j, i));
                }
            }
        }
        else
    ...
    */
    CMatrix& GetCorrelMatrix()
    {
        if (mCorrelInternal.GetNoRows() != mCorrel.GetNoRows())
        {
            for (int i = 0; i < mCorrel.GetNoRows(); i++)
            {
                for (int j = 0; i < mCorrel.GetNoColumns(); j++)
                {
                    mCorrel.SetAt(i, j, mCorrelInternal.GetAt(mBackIndex[static_cast<size_t>(i)], mBackIndex[static_cast<size_t>(j)]));
                    mCorrel.SetAt(j, i, mCorrelInternal.GetAt(mBackIndex[static_cast<size_t>(j)], mBackIndex[static_cast<size_t>(i)]));
                }
            }
        }
        else
    ...
    */
    bool SetCovarMatrix(const CMatrix& mCovarMatrix)
    {
        ...
        {
            // copy all unfixed parameters in the internal list
            for (int i = 0; i < mCovar.GetNoRows(); i++)
            {
                for (int j = 0; j < mCovar.GetNoColumns(); j++)
                {
                    mCovarInternal.SetAt(mBackIndex[static_cast<size_t>(i)], mBackIndex[static_cast<size_t>(j)], mCovar.GetAt(i, j));
                    mCovarInternal.SetAt(mBackIndex[static_cast<size_t>(j)], mBackIndex[static_cast<size_t>(i)], mCovar.GetAt(j, i));
                }
            }
        }
        else
    ...
    */
    CMatrix& GetCovarMatrix()
    {
        if (mCovarInternal.GetNoRows() != mCovar.GetNoRows())
        {
            for (int i = 0; i < mCovar.GetNoRows(); i++)
            {
                for (int j = 0; i < mCovar.GetNoColumns(); j++)
                {
                    mCovar.SetAt(i, j, mCovarInternal.GetAt(mBackIndex[static_cast<size_t>(i)], mBackIndex[static_cast<size_t>(j)]));
                    mCovar.SetAt(j, i, mCovarInternal.GetAt(mBackIndex[static_cast<size_t>(j)], mBackIndex[static_cast<size_t>(i)]));
                }
            }
        }
        else
