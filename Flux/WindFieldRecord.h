...
public:
    ...
    /** Searches through the read-in data and looks for the wind-field at the
            ...
            @return true - if the wind could be interpolated
            @return false - if the wind lies outside of the time-range of the 'database',
                    or the distance between the two data-points to interpolate is larger than 24 hours. */
    bool InterpolateWindField(const CDateTime desiredTime, CWindField &desiredWindField);
