...
{
    return this->m_windField.size();
}

bool CWindFieldRecord::InterpolateWindField(const CDateTime desiredTime, CWindField &desiredWindField) {
    ...
    for (size_t i = 0; i < m_windField.size(); ++i) {
        CWindField &currentRecord = m_windField[i]; // <-- the current record in the database
    ...
    if (foundClosestAfter == false) {
        if (foundClosestBefore == false)
            return false; // should actually not happen
